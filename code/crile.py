...
def Similarity_Set(result):
    ...
    for i in range(result.shape[0]):
        ...
        plt.figure("{}号井 相似井集合--{}曲线".format(wells[i],columname),figsize=(5,10))
...
def Paint_Similarity_Cr(Similarity_Re):
    ...
    for tmp in Similarity_Re:
        ...
        for k in tmp:#range(len(tmp)):
            plt.plot( ydata[k], xdata[k], label = "{}".format(wells[k]) )
        plt.legend()
    #    plt.savefig("{}-AC曲线.jpg".format(string))
    #    plt.show()
        plt.close()

def Paint_example_classification(Similarity_Re):
    i = 0
    plt.figure(u"分类示例图",figsize=(7*len(Similarity_Re),10))
    plt.gca().invert_yaxis()
    plt.axis("off")
    for tmp in Similarity_Re:
        ...
        plt.text(15 + 200*i,1250, "\n".join( [str(wells[item]) for item in tmp ] ) , horizontalalignment="left", verticalalignment = "top")
        i += 1
